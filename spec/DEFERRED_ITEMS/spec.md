...
| 6 | フロント DOM ポーリング (getElementById callsign) | [frontend-refactor-selected-aircraft](../frontend-refactor-selected-aircraft/spec.md) で実装計画あり |
| 8 | スレッドセーフティ (Controller からの setDirectTo 等) | Controller → Service 経由の呼び出しを nextStep と同様に Repository の lock 内で実行するよう設計を見直す |
| 9 | フロント直連 Java API | [frontend-bff](../frontend-bff/spec.md) で Next.js BFF 導入の実装計画あり |
