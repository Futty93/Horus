(() => {
  // Frontend/scripts/aircraftClass.ts
  var Aircraft = class {
    callsign;
    position;
    vector;
  ...
  var GLOBAL_SETTINGS = {
    ...
    isDisplayingRadioNavigationAidsPoint: true,
    isDisplayingAtsLowerRoute: true,
    isDisplayingRnavRoute: true
  };

  // Frontend/scripts/CoordinateManager.ts
  var CoordinateManager = class {
     ...
     */
    calculateCanvasCoordinates(pointLat, pointLon) {
     ...
     */
    calculateFuturePositionOnCanvas(speed, heading, canvasWidth, canvasHeight, displayRange, currentPosition) {
  ...
  function drawPoint(ctx, point, coordinateManager, color, isDisplayingName, isDisplayingPoint) {
  ...
  function drawRoute(ctx, route, coordinateManager, color) {
    const points = route.points;
    for (let i = 0; i < points.length - 1; i++) {
      drawLineBetweenPoints(ctx, points[i], points[i + 1], coordinateManager, color);
    }
  }
  function drawLineBetweenPoints(ctx, point1, point2, coordinateManager, color) {
  ...
  var RadarGame = class {
    ...
    drawLabelLine(index, airplane) {
      const position = airplane.position;
      const labelX = position.x + airplane.label.x;
      const labelY = position.y - airplane.label.y;
      const labelDistance = Math.sqrt(
        Math.pow(airplane.label.x, 2) + Math.pow(airplane.label.y, 2)
      );
    ...
    drawLabel(index, airplane) {
      ...
      const labelX = airplanePosition.x + airplane.label.x;
      const labelY = airplanePosition.y - airplane.label.y;
      this.ctx[index].fillStyle = "white";
    ...
    drawWaypoint(index, name, latitude, longitude) {
      const radius = 5;
      const sides = 5;
      ...
      this.ctx[index].stroke();
      const textX = latitude + radius * Math.cos(angle / 2);
      const textY = longitude - radius;
  ...
  var fetchAircraftLocation = async () => {
    try {
      const response = await fetch(
      ...
      if (response.ok) {
        ...
        if (aircraftData) {
          updateControlledAirplanes(aircraftData);
        } else {
  ...
  function updateControlledAirplanes(apiResponse) {
  ...
  startButton?.addEventListener("click", () => {
    ...
    if (!locationUpdateInterval) {
      locationUpdateInterval = setInterval(fetchAircraftLocation, 1e3);
    }
  });
