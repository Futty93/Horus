...
import { DisplaySettings } from "@/context/routeInfoDisplaySettingContext";
...
import {
  NM_TO_KM,
  nmToCanvasRadiusPx,
} from "@/utility/rangeRings/rangeRingsUtils";
...
export function renderMap(
  ...
  if (displaySettings.atsLowerRoute) {
    // GLOBAL_SETTINGSではなくdisplaySettingsを使用
    [...atsRoutes].forEach((route) => {
      drawRoute(ctx, route, "#0ff", centerCoordinate, displayRange);
    });
  }
  if (displaySettings.rnavRoute) {
    // GLOBAL_SETTINGSではなくdisplaySettingsを使用
  ...
  [...waypoints].forEach((point) => {
    drawPoint(
      ...
      point,
      "#376",
      displaySettings.waypointName,
      displaySettings.waypointPoint,
      centerCoordinate,
      displayRange
    ); // GLOBAL_SETTINGSではなくdisplaySettingsを使用
  });
  [...radioNavAids].forEach((point) => {
    drawPoint(
      ...
      point,
      "#376",
      displaySettings.radioNavigationAidsName,
      displaySettings.radioNavigationAidsPoint,
      centerCoordinate,
      displayRange
    );
  });
...
export function drawRangeRings(
  ...
  const canvasWidth = GLOBAL_SETTINGS.canvasWidth;
  ...
  for (
    let rNm = settings.intervalNm;
    rNm <= maxRadiusNm;
    rNm += settings.intervalNm
  ) {
    const radiusPx = nmToCanvasRadiusPx(rNm, canvasWidth, rangeKm);
    ctx.beginPath();
...
function drawPoint(
  ctx: CanvasRenderingContext2D,
  point: Waypoint | RadioNavigationAid,
  color: string,
  isDisplayingName: boolean,
  isDisplayingPoint: boolean,
  centerCoordinate: Coordinate,
  displayRange: DisplayRange
) {
  const { x, y } = CoordinateManager.calculateCanvasCoordinates(
    point,
    centerCoordinate,
    displayRange
  ); // GLOBAL_SETTINGSではなくcenterCoordinateを使用
  ...
  if (isDisplayingPoint) {
    ...
    if (point["type"] !== "waypoint") {
      // Checking if it's a RadioNavigationAid
...
function drawLineBetweenPoints(
  ...
  const point1Coordinate = CoordinateManager.calculateCanvasCoordinates(
    point1,
    centerCoordinate,
    displayRange
  );
  const point2Coordinate = CoordinateManager.calculateCanvasCoordinates(
    point2,
    centerCoordinate,
    displayRange
  );
