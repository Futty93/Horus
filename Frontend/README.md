...
- 航空機の位置表示
- **履歴ドット**: 位置更新のたびに直前の緯度・経度・高度（WGS-84）をクライアント側に蓄積（最大 **120** 点）。**描画は 30s / 60s / 90s 前**に相当するサンプルを各 1 点ずつ選び、`CoordinateManager.calculateCanvasCoordinates` でキャンバス座標へ変換する（パン・中心座標・表示範囲の変更後も軌跡がずれない）。点の半径は `TRACK_DISPLAY_DOT_RADIUS_PX`（既定 **3px**、機体マーカー 5px より小さく）。
